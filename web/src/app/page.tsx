...
function NameLink({ p }: { p: Process }) {
  ...
  return (
    <Link href={`/process/${p.id}`}
      className={`group inline-flex items-center gap-1 font-medium hover:underline ${blue ? "text-blue-600 dark:text-blue-400" : ""}`}>
      {p.name}
      <span className="text-slate-300 opacity-0 transition-opacity group-hover:opacity-100 dark:text-neutral-600">›</span>
    </Link>
  );
}

const cardCls = "overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-900";
const thR = "px-2 py-1.5 text-right font-medium text-slate-400 dark:text-neutral-500";
const tdR = "px-2 py-1.5 text-right tabular-nums";

function CardHeader({ title, accent, count, right }: { title: string; accent: string; count: number; right: string }) {
  return (
    <header className="flex items-center justify-between gap-2 border-b border-slate-100 px-3 py-2 dark:border-neutral-800">
      <div className="flex items-center gap-2">
        ...
        <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] text-slate-500 dark:bg-neutral-800 dark:text-neutral-400">{count}</span>
      </div>
...
function ErrCell({ v }: { v: number }) {
  ...
  return (
    <td className={tdR}>
      <span className={`rounded-md px-1.5 py-0.5 ${e !== 0 ? "bg-rose-50 font-semibold text-rose-600 dark:bg-rose-950/40 dark:text-rose-400" : "text-slate-300 dark:text-neutral-600"}`}>
        {fmtWeight(e)}
      </span>
    </td>
  );
...
function ProcessCard({ title, accent, procs, agg }: { title: string; accent: string; procs: Process[]; agg: Map<string, Agg> }) {
  ...
  return (
    ...
    <section className={`${cardCls} flex min-w-[360px] flex-1 flex-col`}>
      <CardHeader title={title} accent={accent} count={procs.length} right={`재고 ${fmtWeight(tStock)}`} />
      <table className="w-full flex-1 text-xs">
        ...
        <tbody className="divide-y divide-slate-50 dark:divide-neutral-800/60">
          {procs.length === 0 ? (
            <tr><td colSpan={6} className="px-2 py-6 text-center text-slate-300 dark:text-neutral-600">데이터 없음</td></tr>
          ) : procs.map((p) => {
            const a = A(p.id);
            ...
            return (
              <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-neutral-800/50">
                ...
                <td className={`${tdR} text-slate-500 dark:text-neutral-400`}>{fmtWeight(a.lossW)}</td>
                <ErrCell v={a.inW - a.stock - a.outW - a.lossW} />
              </tr>
            );
          })}
          {/* 남는 세로 공간 흡수 → 계 행을 바닥으로 밀어 18K/14K 카드 간 정렬 */}
          <tr aria-hidden className="h-full"><td colSpan={6} /></tr>
        </tbody>
        <tfoot>
...
function FlowCard({ title, accent, label, procs, agg }: { title: string; accent: string; label: string; procs: Process[]; agg: Map<string, Agg> }) {
  const A = (id: string) => agg.get(id) ?? EMPTY;
  const tIn = round2(procs.reduce((a, p) => a + A(p.id).inW, 0));
  const tOut = round2(procs.reduce((a, p) => a + A(p.id).outW, 0));
  return (
    ...
    <section className={`${cardCls} flex min-w-[230px] flex-1 flex-col`}>
      <CardHeader title={title} accent={accent} count={procs.length} right={`입 ${fmtWeight(tIn)}`} />
      <table className="w-full flex-1 text-xs">
        <thead>
          <tr className="border-b border-slate-100 dark:border-neutral-800">
        ...
        <tbody className="divide-y divide-slate-50 dark:divide-neutral-800/60">
          {procs.length === 0 ? (
            <tr><td colSpan={3} className="px-2 py-6 text-center text-slate-300 dark:text-neutral-600">데이터 없음</td></tr>
          ) : procs.map((p) => {
            const a = A(p.id);
            return (
              <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-neutral-800/50">
          ...
          <tr aria-hidden className="h-full"><td colSpan={3} /></tr>
        </tbody>
        <tfoot>
          <tr className="border-t border-slate-200 bg-slate-50/60 font-semibold dark:border-neutral-700 dark:bg-neutral-800/40">
            <td className="px-2 py-1.5">계</td>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="space-y-2">
      <h2 className="text-xs font-semibold uppercase tracking-wide text-slate-400 dark:text-neutral-500">{title}</h2>
...
export default async function Home() {
    ...
    return (
      <main className="mx-auto max-w-2xl p-8">
        ...
        <div className="rounded-xl border border-amber-400 bg-amber-50 p-4 text-sm dark:bg-amber-950/40">
          <p className="mb-2 font-semibold">⚙️ Supabase 연결이 아직 설정되지 않았습니다.</p>
  ...
  const procList = ((procData ?? []) as Process[]).filter((p) => p.schema_type !== "entry");
  const dept = (k: Karat) => procList.filter((p) => p.schema_type === "io" && !p.is_inspection && p.karat === k);
  const work = (k: Karat) => procList.filter((p) => p.schema_type === "work" && p.karat === k);
  const insp = (k: Karat) => procList.filter((p) => p.is_inspection && p.karat === k);

  const sumStock = (procs: Process[]) => round2(procs.reduce((a, p) => a + (agg.get(p.id)?.stock ?? 0), 0));
  ...
  return (
    <main className="space-y-5 p-6">
      ...
      <Section title="공정 (연마·뻥·빠우)">
        ...
        <ProcessCard title="14K 공정" accent="bg-blue-500" procs={work("14K")} agg={agg} />
      </Section>

      <Section title="부서 · 검수">
        <FlowCard title="18K 부서" accent="bg-rose-500" label="부서" procs={dept("18K")} agg={agg} />
        <FlowCard title="14K 부서" accent="bg-blue-500" label="부서" procs={dept("14K")} agg={agg} />
        <FlowCard title="18K 검수" accent="bg-rose-500" label="검수" procs={insp("18K")} agg={agg} />
        <FlowCard title="14K 검수" accent="bg-blue-500" label="검수" procs={insp("14K")} agg={agg} />
      </Section>

      <p className="text-[11px] text-slate-400 dark:text-neutral-500">
        ※ 공정 오차 = 입고 − 재고 − 출고 − 로스 (정상이면 0, <span className="text-rose-500">빨강</span>은 집계 불일치).
        재고 = 입고(작업중 잠금 전) 중량 합. 미출고 = 작업완료(side=out) 후 아직 출고·이관(잠금) 안 된 건.
      </p>
    </main>
  );
}
