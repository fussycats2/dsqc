...
Public Sub 선택행_LY_삭제_위로올림()
    ...
    ' === 선택 범위를 L:Y로 한정 (2단계 체크) ===
    Set onlyLY = Intersect(sel, ws.Range("L:Y"))
    If onlyLY Is Nothing Then
        MsgBox "올바른 셀을 선택하세요." & vbCrLf & _
               "L~Y 열 범위 내 셀만 선택한 상태에서 실행할 수 있습니다." & vbCrLf & _
               "현재 선택: " & sel.Address(0, 0), vbExclamation, "선택 범위 제한"
        Exit Sub
    End If
    If onlyLY.CountLarge <> sel.CountLarge Then
        MsgBox "올바른 셀을 선택하세요." & vbCrLf & _
               "L~Y 열 범위 내 셀만 선택한 상태에서 실행할 수 있습니다." & vbCrLf & _
               "현재 선택: " & sel.Address(0, 0), vbExclamation, "선택 범위 제한"
        Exit Sub
    ...
    ' 대상: (선택된 행들) ∩ (L:Y 열)
    Set Target = Intersect(rowsUnion, ws.Columns("L:Y"))
    If Target Is Nothing Then
        MsgBox "선택된 행에 L~Y 범위가 없습니다.", vbInformation
        Exit Sub
    End If
    ...
    resp = MsgBox( _
        "정말 삭제할까요?" & vbCrLf & _
        "대상: " & rowCnt & "개 행의 L:Y 범위" & vbCrLf & _
    ...
    MsgBox rowCnt & "개 행의 L:Y를 삭제(위로 올림)했습니다.", vbInformation
    Exit Sub
