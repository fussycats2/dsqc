...
' === 빠른 배치 인쇄(L11:Y{last}) : 여러 시트를 한 번에 인쇄 (선택 로직 안정화) ===
Private Sub FastBatchPrint_L11Y(sheetNames As Variant, ByVal Preview As Boolean)
    ...
    ' 1) 대상 시트 수집(존재 + L11:U에 데이터 있는 것만)
    ...
    For i = 1 To cnt
        ...
        With ws.PageSetup
            .PrintArea = "$L$11:$Y$" & lastRows(i)   ' 인쇄범위: L~Y (lastRow는 L~U 기준)
            .Zoom = False
            .FitToPagesWide = 1
...
' === 메인: 출고장부 일괄 인쇄(미리보기 기본) ===
Public Sub 인쇄_출고장부_일괄(Optional ByVal Preview As Boolean = True)
    ...
    ask = MsgBox("출고 장부를 출력하시겠습니까?" & vbCrLf & _
                 "대상 시트: " & listText, _
    ...
    ' ★ 빠른 배치 인쇄 호출
    Call FastBatchPrint_L11Y(sheetNames, Preview)
...
' === Alt+F8용 래퍼 ===
Public Sub 인쇄_출고장부_일괄_미리보기()
    인쇄_출고장부_일괄 True
End Sub

Public Sub 인쇄_출고장부_일괄_바로인쇄()
    인쇄_출고장부_일괄 False
End Sub
